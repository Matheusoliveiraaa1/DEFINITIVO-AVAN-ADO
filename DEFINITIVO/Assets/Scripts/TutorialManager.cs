...
public class TutorialManager : MonoBehaviour
{
    ...
    [Header("Controle do Botão de Tutorial")]
    public GameObject objetoQueBloqueiaTutorial; // GameObject que será monitorado
    public Button botaoAbrirTutorial; // Botão que abre o tutorial manualmente
    ...
    private bool positionsCaptured = false; // Nova variável

    void Awake()
    {
        CapturePositions(); // Captura logo ao ligar o jogo
    ...
    void Update()
    {
        if (objetoQueBloqueiaTutorial != null && botaoAbrirTutorial != null)
        {
            botaoAbrirTutorial.gameObject.SetActive(!objetoQueBloqueiaTutorial.activeSelf);
        }
    }
    ...
    IEnumerator StartTutorialWithDelay()
    {
        ...
        // REMOVI AS LINHAS DE CÁLCULO DAQUI (ELAS ESTÃO NO CAPTUREPOSITIONS AGORA)
    ...
    IEnumerator FirstSlideEnter()
    {
        ...
        while (elapsed < firstSlideEnterDuration)
        {
            ...
            rect.anchoredPosition =
                Vector3.Lerp(firstSlideOffScreenRight, firstSlideOriginalPos, t);
    ...
    IEnumerator FirstSlideIdle()
    {
        ...
        while (true)
        {
            ...
            rect.anchoredPosition =
                firstSlideOriginalPos + new Vector3(offsetX, offsetY, 0);
    ...
    IEnumerator AnimateProfessoraEntrance()
    {
        ...
        while (elapsedTime < slideInDuration)
        {
            ...
            rect.anchoredPosition =
                Vector3.Lerp(offScreenPosition, originalImagePosition, t);
    ...
    IEnumerator AnimateProfessoraIdle()
    {
        ...
        while (true)
        {
            ...
            float smoothX = Mathf.Lerp(
                rect.anchoredPosition.x - originalImagePosition.x,
                targetX,
                Time.deltaTime * 2f
            );

            float smoothY = Mathf.Lerp(
                rect.anchoredPosition.y - originalImagePosition.y,
                targetY,
                Time.deltaTime * 2f
            );

            rect.anchoredPosition =
                originalImagePosition + new Vector3(smoothX, smoothY, 0);
            ...
            float smoothRotation =
                Mathf.Lerp(currentRotation, targetRotation, Time.deltaTime * 3f);
    ...
    IEnumerator AnimateExtraImage()
    {
        ...
        while (true)
        {
            ...
            float smoothX =
                Mathf.Lerp(rect.anchoredPosition.x - originalPos.x, offsetX, Time.deltaTime * 2f);

            float smoothY =
                Mathf.Lerp(rect.anchoredPosition.y - originalPos.y, offsetY, Time.deltaTime * 2f);

            rect.anchoredPosition =
                originalPos + new Vector3(smoothX, smoothY, 0);
    ...
    public void OpenTutorialManually()
    {
        ...
        firstSlideAlreadyShown = false;
        isTyping = false;

        

        // Apenas resetamos para os valores que foram salvos no Awake
        professoraImage.rectTransform.anchoredPosition = originalImagePosition;
        professoraImage.rectTransform.localRotation = Quaternion.identity;

        firstSlideImage.rectTransform.anchoredPosition = firstSlideOriginalPos; 
        firstSlideImage.rectTransform.localRotation = Quaternion.identity;
        firstSlideImage.gameObject.SetActive(false);

        extraImageUI.rectTransform.localRotation = Quaternion.identity;
        tutorialText.text = "";
    ...
    void CapturePositions()
    {
        ...
        // Salva a posição real que está no Inspector
